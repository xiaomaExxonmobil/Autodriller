...
model_name = "AD_Health"
input_table_name = "sandbox.auto_driller_ml_feature_table"
output_table_path = "/FileStore/batch-inference/AD_Health"
...
output_df = output_df.withColumn('AD_Health', when(col('prediction')<0.5, 0).otherwise('1'))
output_df = output_df.withColumn('AD_Health_Final', when(col('AD_Health')==0, 'good').otherwise('bad'))
...
write_table_name = 'auto_driller_inference_mvp_health'
write_table_location = 'sandbox'
mnt_location =f"{output_table_path}_{datetime.now().isoformat()}".replace(":", ".") 
